...
export const exportAttendance = catchAsync(async (req: AuthRequest, res: Response, next: NextFunction) => {
    ...
    // verify password
    const user = await require('../models/User').default.findById(req.user.id).select('+passwordHash');
